...
// @version           0.0.1
// @description       残月vip视频解析【内嵌解析+带弹幕】，体验腾讯、爱奇艺、优酷、芒果、等vip视频网站
...
const superVip = (function () {
    ...
    const _CONFIG_ = {
        ...
        videoParseList: [
            {"name": "解析线路1", "type": "1,3", "url": "https://im1907.top/?jx="},
            {"name": "解析线路2", "type": "1,3", "url": "https://jx.xmflv.com/?url="},
            {"name": "解析线路3", "type": "1,3", "url": "https://dm.xmflv.com:4433/?url="},
        ],
        playerContainers: [
            {
                host: "v.qq.com",
    ...
    class BaseConsumer {
        ...
        generateElement(container) {
            ...
            let autoPlay = !!GM_getValue(_CONFIG_.autoPlayerKey, null) ? "自动" : "手动";
 
            $(container).append(`
                <div id="${_CONFIG_.vipBoxId}">
                    <div class="vip_icon">
                        <div class="img_box" title="选择解析源" style="color:white;font-size:16px;font-weight:bold;border-radius:5px;"><span style="color: yellow;">解析</span></div>
                        <div class="vip_list">
                            <div>
                                <h3 style="color:#1c84c6; font-weight: bold; font-size: 16px; padding:5px 0px;">残月vip视频解析</h3>
                                <ul>
                                    ${type_1_str}
                            ...
                            <div style="text-align:left;color:#FFF;font-size:10px;padding:0px 10px;margin-top:10px;">
                                <b>解析说明：</b>
                                <br>&nbsp;&nbsp;1、内嵌弹幕自动集成，可调整弹幕样式
                                <br>&nbsp;&nbsp;2、若解析失败，请尝试切换其他线路
                                <br>&nbsp;&nbsp;3、更多实用脚本<a href="https://greasyfork.org/zh-CN/users/1182757-bsutss" target="_blank"><span style="color:#E56600;">请点击这里查看</span></a> 
                            </div>
                        </div>
                    </div>
